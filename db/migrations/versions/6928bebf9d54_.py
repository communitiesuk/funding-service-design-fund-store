...
english_to_welsh_title = {
    ...
    for key, value in {
        "funding to save an asset in your community": (
            "cyllid i achub ased yn eich cymuned"
        ),
    }.items()
}
...
def upgrade():
    ...
    query = sqlalchemy.text(
        "SELECT id, name_json, title_json, description_json FROM fund WHERE"
    ...
    for id, name_json, title_json, description_json in funds:
        ...
        update_query = sqlalchemy.text(
            f"UPDATE fund SET name_json = '{new_name_json}' WHERE id = '{id}'"
        )
        ...
        update_query = sqlalchemy.text(
            f"UPDATE fund SET title_json = '{new_title_json}' WHERE id = '{id}'"
        )
        connection.execute(update_query)

        english_description = description_json["en"]
        welsh_description = english_to_welsh_description.get(
            english_description.casefold()
        )
        if not welsh_description:
            raise Exception(
                f"Could not find welsh description for {english_description}"
            )
        description_json["cy"] = welsh_description
        new_description_json = json.dumps(description_json)
        update_query = sqlalchemy.text(
            f"UPDATE fund SET description_json = '{new_description_json}' WHERE id ="
            f" '{id}'"
        )
        connection.execute(update_query)

    query = sqlalchemy.text(
        "SELECT id, title_json FROM round WHERE fund_id ="
    ...
    for id, title_json in funds:
        ...
        new_title_json = json.dumps(title_json)
        update_query = sqlalchemy.text(
            f"UPDATE round SET title_json = '{new_title_json}' WHERE id = '{id}'"
        )
        connection.execute(update_query)
