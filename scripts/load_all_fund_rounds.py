...
def load_all_fund_rounds() -> None:
    ...
    loader_module_names = [f for f in os.listdir("scripts/fund_round_loaders") if f.endswith(".py")]
    for module_name in loader_module_names:
        # Remove the ".py" extension to get the module name
        module_name_without_extension = module_name[:-3]

        # Dynamically import the module
        loader_module = importlib.import_module(f"scripts.fund_round_loaders.{module_name_without_extension}")
